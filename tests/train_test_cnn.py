...
if __name__ == '__main__':
    ...
    # Get the preprocessed and segmented data from the large file to use as training. With hop=0.01, one point every 10 ms
    seg_train_data, train_labels, _ = train_file.get_segmented_data(segment_size_sec=0.06, hop=0.01,
                                                                    filter_to_apply=butter_filter)

    # Get test dataset from the other file to use as test
    seg_test_data, Y_test, test_xtime = test_file.get_segmented_data(segment_size_sec=0.06, hop=0.01,
                                                                     filter_to_apply=butter_filter)
